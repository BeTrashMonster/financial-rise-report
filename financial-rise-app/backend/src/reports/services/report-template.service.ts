...
/**
 * Service for rendering HTML templates for consultant and client reports
 * Ported from Express backend - implements DISC-adapted report generation
 */
@Injectable()
export class ReportTemplateService {
  ...
  public renderConsultantReport(data: ConsultantReportData): string {
    this.logger.log(`Rendering consultant report for assessment ${data.assessment.id}`);
...
<body>
  ...
  public renderClientReport(data: ClientReportData): string {
    this.logger.log(`Rendering client report for ${data.client.name}`);
